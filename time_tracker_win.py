...
def new_sheet():
    df = pd.DataFrame(
        data=None, columns=["date", "start", "break", "end", "total"]
    )
    total_row = {
        "date": time.strftime("%B_%y", time.localtime()).lower(),
        "total": 0,
    }
    df = pd.concat([df, pd.DataFrame([total_row])], axis=0, ignore_index=True)
    # Safe the df if it's the first time
    df.to_csv(
        f"timesheets_csv/timesheet_{sheet_spec}_{time.strftime('%B', time.localtime()).lower()}.csv",
        index=False,
    )
...
def time_disp():
    time_label = ttk.Label(window, font=("calibri", 20, "bold"))
    time_label.place(x=time_x, y=time_y * 4)

    break_duration = time.gmtime(
        (datetime.now() - break_list[0]).total_seconds()
    )
...
def start_tracking(event):
    ...
    global break_start
    break_start = ttk.Button(
        window, text="Start break", command=lambda: start_break()
    )
    break_start.place(x=break_x, y=break_y)

    quit = ttk.Button(window, text="Quit", command=lambda: exit())
...
def start_break():
    ...
    global break_end
    break_end = ttk.Button(
        window, text="End break", command=lambda: end_break()
    )
...
def exit():
    ...
    save_window.geometry(
        "%dx%d+%d+%d" % (save_width, save_heigth, (x + 3), (y + 27))
    )
    ...
    cancel = ttk.Button(
        save_window, text="Cancel", command=lambda: save_window.destroy()
    )
    cancel.place(x=time_x, y=time_y)

    save_exit = ttk.Button(
        save_window, text="Save & Exit", command=lambda: save_on_exit()
    )
    save_exit.place(x=break_x, y=break_y)

    exit = ttk.Button(save_window, text="Exit", command=lambda: no_save_exit())
...
def save_on_exit():
    ...
    try:
        ...
        new_entry["total"] = time.strftime(
            "%H:%M", time.gmtime(effective_work.total_seconds())
        )
        # Add the break time (if there is no break add 0)
        try:
            new_entry["break"] = time.strftime(
                "%H:%M", time.gmtime(sum(breaks))
            )
        except:
            new_entry["break"] = "00:00"
        # Appned new entry as row
        df = pd.concat(
            [df, pd.DataFrame([new_entry])], axis=0, ignore_index=True
        )
        ...
        df["total"][
            0
        ] = f"{int(total_work / 60 / 60)}:{int(total_work / 60 % 60)}"
        # Save
        df.to_csv(
            f"timesheets_csv/timesheet_{sheet_spec}_{time.strftime('%B', time.localtime()).lower()}.csv",
            index=False,
        ...
        df.to_excel(
            f"timesheets_xls/timesheet_{sheet_spec}_{time.strftime('%B', time.localtime()).lower()}.xlsx",
            index=False,
        )
    except:
        pass
