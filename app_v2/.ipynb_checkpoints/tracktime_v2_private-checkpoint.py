...
def load_sheet():
    try:
        return pd.read_csv(
            f"{path}timesheets_csv/{sheet_spec}/timesheet_{sheet_spec}_{time.strftime('%B', time.localtime()).lower()}.csv"
        )
...
if platform == "linux" or platform == "linux2":
    if os.uname()[1] == 'Erebor':
        canvas_spec = 1
        font_size_time = 7
        relx_change_start = .19
        x_adjust = 3
        y_adjust = 71
    else:
        canvas_spec = 1
...
sheet_spec_field.place(in_=window, relx=.1, rely=.45, anchor="nw", relwidth=.2)
...
try:
    ...
    path_field.pack()
    path_field.place(in_=window, relx=.1, rely=.65, anchor="nw", relwidth=.8)
...
def systray_save_exit(icon, item):
    ...
    if new_entry["date"] != df["date"][last_index]:
        ...
        try:
            ...
            # Save
            if not os.path.exists(f"{path}timesheets_xls/{sheet_spec}"):
                os.makedirs(f"{path}timesheets_xls/{sheet_spec}")
                
            if not os.path.exists(f"{path}timesheets_csv/{sheet_spec}"):
                os.makedirs(f"{path}timesheets_csv/{sheet_spec}")
            ...
            df.to_excel(
                f"{path}timesheets_xls/{sheet_spec}/timesheet_{sheet_spec}_{time.strftime('%B', time.localtime()).lower()}.xlsx",
                index=False,
            )
...
def tray_icon():
    ...
    menu = (
        ...
        item("Save", systray_save_exit),
        item("Quit", systray_exit),
    )

    icon = pystray.Icon("name", tray_image, f"TrackTime: '{sheet_spec}'", menu)
